...
const Camera = ({
    ...
    set_region: (r: string) => void, region: string,
    floor: string, set_floor: (r: string) => void,
    ...
    const point_inside_limits = useCallback((point: [number, number, number]): { name: 'unknown' } | limit_place => {
        ...
        for (let place of floor_limits) {
            ...
            for (let i = 0, j = polygon.length - 1; i < polygon.length; j = i++) {
                let xi = polygon[i][0], zi = polygon[i][1]
                let xj = polygon[j][0], zj = polygon[j][1]
        ...
        const place: limit_place | { name: 'unknown' } = insides.some(t => t) ? (floor_limits[insides.indexOf(true)]) : { name: 'unknown' }
    ...
    useFrame(() => {
        ...
        const place = point_inside_limits([camera.position.x, camera.position.y, camera.position.z])
        ...
        if (place.name == 'unknown') {
            ...
            new TWEEN.Tween(camera.position).to({ y: (place as { y: number }).y }, 150).easing(TWEEN.Easing.Sinusoidal.InOut).start()
        }

        const x_min = -237.98529838684982, z_min = -266.4529615979886
        const x_max = 162.55989823299316, z_max = 111.03816240392462
    ...
    useEffect(() => {
        ...
        for (let [name, place] of Object.entries(limits[floor])) {
            if (name == 'vein A' || name == 'vein B' || name == 'vein C' || name == 'vein D') {
    ...
    return (<>
        ...
        }} selector={"#app"} ref={controls as any} />
        <camera />
    </>)
}
